...
class QueueStreamPattern(
    brokerAddress: String,
    config: PatternConfig,
) : BasePattern("queue_stream", brokerAddress, config) {
    ...
    override suspend fun run() {
        ...
        for (ch in channelNames) {
            scope.launch {
                while (running) {
                    try {
                        val response = receiver!!.receiveQueuesMessages {
                            channel = ch
                            maxItems = 10
                            waitTimeoutMs = 5000
                            autoAck = true
        ...
        for (ch in channelNames) {
            scope.launch {
                rateLimitedLoop(this, config.rate) {
                    try {
                        sender!!.sendQueuesMessage(queueMessage {
                            channel = ch
                            metadata = "burnin"
    ...
    private fun createClient(suffix: String): QueuesClient = KubeMQClient.queues {
        ...
        clientId = "burnin-queue-stream-$suffix"
    }
